...
def edit(request):
    ...
    obj = models.SalesTeam.objects.filter(id=uid).first()
    if not obj:
...
def edit_detail(request):
    """ 获取要编辑的数据 """
    uid = request.GET.get("uid")
    obj_dict = models.SalesTeam.objects.filter(id=uid).values().first()  # 获得的是一个字典
    if not obj_dict:
...
def delete(request):
    """ 删除数据 """
    uid = request.GET.get("uid")
    if not models.SalesTeam.objects.filter(id=uid).exists():
        return JsonResponse({"status": False, "error": "删除失败，数据不存在"})
    models.SalesTeam.objects.filter(id=uid).delete()
    return JsonResponse({"status": True})
...
def delete_all(request):
    ...
    if md5(pwd) == password:
        models.SalesTeam.objects.all().delete()
        return JsonResponse({"status": True})

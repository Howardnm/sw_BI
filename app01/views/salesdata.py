...
def salesdata_list(request):
    ...
    search_bar = SearchBar(request, form.fields.keys())
    queryset = models.SalesData.objects.filter(**search_bar.filter_contains()).order_by("id")
    page_obj = Pagination(request, queryset, "page", 12)
    context = {
        "search_form": search_bar.html(),  # 搜索框保留搜索值
